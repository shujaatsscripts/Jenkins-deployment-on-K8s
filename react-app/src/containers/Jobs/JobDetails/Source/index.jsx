...
const ListCandidates = () => {
  ...
  const [activeSource, setActiveSource] = useState("Portal");
  ...
  const Cards = ({ data }) => {
    ...
    Object.keys(data)?.map((source, index) => {
      cards.push(
        ...
        <Col xs={22} sm={23} md={4} key={index}>
          <a onClick={() => handleSourceClick(source)}>
            <StyledBox
...
const StyledBox = styled.div`
  ...
  align-items: center;
  /* height: 150px; */
  background-color: ${(props) => (props.active ? "#1a77f2" : null)};
